...
export default function TagPageHeader({ tag, count }: TagPageHeaderProps) {
  ...
  if (!mounted) {
    return (
      <div className="mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-red-100 text-red-700 rounded-full text-sm font-bold mb-3">
          #{tag}
        </div>
        <h1 className="text-2xl sm:text-3xl font-black text-gray-900">
          &quot;{tag}&quot; से जुड़ी खबरें
        </h1>
        <p className="text-sm text-gray-500 mt-1">{count} खबरें मिलीं</p>
      </div>
    );
  }
